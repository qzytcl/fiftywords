...
var Util = {
    ...
    solar2lunar:function (y,m,d) { //参数区间1900.1.31~2100.12.31
        ...
        festival = this.getLunarFestival(month, day)
        if (!festival) {
          festival = this.getFestival(m, d)
        }
        ...
        'astro':astro,
        'festival': festival,
        'metaphysics':this.getSuitAndTaboo(year,month,day)};
    },
  /**
 * 玄学啊,获取禁忌事情
 * @param year
 ...
 */
  getSuitAndTaboo: function (year, month, day) {
    ...
    var dateString = parseInt((year * month * day) % 1024).toString(2);
  ...
  getLunarFestival(month,day){
      ...
      let key = this.getFestivalKey(month,day);
      let result = festivalName[key]
      return result
    },
    getFestivalKey(month,day){
      if (month > 0 && month < 13 && day > 0 && day < 32) {
          var festivalNumber = '';
          if (month < 10)
            festivalNumber = "0" + month;
          else
            var festivalNumber = month;
          if (day < 10)
            festivalNumber += "0" + day;
          else
            festivalNumber += day;  //拼接字符串
    ...
    getFestival(month,day){
      ...
      return festivalName[this.getFestivalKey(month, day)]; 
    },
    /**
